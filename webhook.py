...
def ontvangen():
    ...
    LEADS.insert(0, lead)
    log.info(f"Opgeslagen: {naam} {tier}")
...
def dashboard():
    ...
    rows = ""
    for l in LEADS:
        badge_color = {"HOT": "#ea4335", "WARM": "#fa7b17", "COLD": "#4285f4"}.get(l["tier"], "#888")
        tmpl = str(l.get("whatsapp_template", ""))[:80]
        rows += (
            "<tr>"
            + f"<td>{l['tijdstip']}</td>"
            + f"<td><strong>{l['naam']}</strong></td>"
            + f"<td>{l['telefoon']}</td>"
            + f"<td>{l['postcode']}</td>"
            + f"<td>{l['dienst']}</td>"
            + f"<td>{l['score']}</td>"
            + f'<td><span style="background:{badge_color};color:#fff;padding:3px 10px;border-radius:12px;font-weight:bold">{l["tier"]}</span></td>'
            + f"<td title='{str(l.get('whatsapp_template',''))}'>{tmpl}...</td>"
            + "</tr>"
        )
    html = (
        "<!DOCTYPE html><html><head><meta charset='UTF-8'>"
        "<title>NMT Lead Dashboard</title>"
        "<style>body{font-family:Arial,sans-serif;background:#0f1923;color:#fff;padding:20px;margin:0}"
        "h1{color:#f97316}.stats{display:flex;gap:16px;margin:20px 0}"
        ".stat{background:#1a2744;border-radius:8px;padding:16px 24px;text-align:center}"
        ".stat .n{font-size:2em;font-weight:bold}.hot{color:#ea4335}.warm{color:#fa7b17}.cold{color:#4285f4}"
        ...
        "tr:hover td{background:#1a2744}</style></head><body>"
        "<h1>NMT Lead Dashboard</h1>"
        f"<p style='color:#888'>{len(LEADS)} leads</p>"
        "<div class='stats'>"
        f"<div class='stat'><div class='n hot'>{hot}</div><div>HOT</div></div>"
        f"<div class='stat'><div class='n warm'>{warm}</div><div>WARM</div></div>"
        f"<div class='stat'><div class='n cold'>{cold}</div><div>COLD</div></div>"
        "</div>"
        "<table><thead><tr>"
